...
@pytest.mark.asyncio
async def test_same_meta_url_val_record(
    wiped_database,
    operator: SyncAgenciesTaskOperator,
    db_data_creator: DBDataCreator
):
    """
    Test that, in the case of a Meta URL already existing in the DB
     and linked to an agency but having:
    - A URLValidationFlag that is not `Meta URL`
    - A Record Type that is not `Contact Info and Agency Meta`
    The Meta URL should have:
    - The URLValidationFlag set to `Meta URL`
    - The Record Type set to `Contact Info and Agency Meta`
    - The link to the agency untouched
    """
    ...
    # Create URL and link to Agency
    url_mapping: URLMapping = (await db_data_creator.create_validated_urls(
    ...
    # Confirm 1 URLValidationFlag in database
    ...
    assert all(flag.url_id == url_id for flag in flags)

    # Confirm 1 Agency-URL Link
